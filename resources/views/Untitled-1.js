...
<script>
    $(document).ready(function() {
        $.ajax({
            ...
            success: function(data) {
                ...
                // console.log('Digest Value2:', computedDigest);
  ...
  return btoa(binary);
    ...
    async function calculateHmacSha256(signingKeyString, signatureString) {

        console.log("signingKeyString ", signingKeyString);
        console.log("signatureStrings ", signatureString);

        // Step 2: Convert the signatureString to bytes
        const encoder = new TextEncoder();
        const dataBytes = encoder.encode(signatureString);

        console.log("UTF  "+dataBytes);

        const keyBuffer = encoder.encode(signingKeyString);
        ...
        const resultBytes = await crypto.subtle.sign('HMAC', cryptoKey, dataBytes);

        console.log("resultBytes "+resultBytes);

        // Step 4: Encode the result in Base64
        //   const encodedResult = btoa(String.fromCharCode.apply(null, new Uint8Array(resultBytes)));
        const encodedResult = arrayBufferToBase64(resultBytes);
    ...
    function computeHttpSignature(config, headerHash) {
        ...
        var hashf = (function() {
            ...
            let decodedString = btoa(signingBase);
        ...
        // var hash = hashf(decodedString, signatureString);


        var getSignature = calculateHmacSha256(decodedString, config.secretkey );
        console.log("signature  " + JSON.stringify(getSignature));
